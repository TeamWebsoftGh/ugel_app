<form method="POST" action="{{route('meetings.store')}}" enctype="multipart/form-data">
    ...
    @csrf
    <input type="hidden" id="_id" name="id" value="{{$meeting->id}}">
    <input type="hidden" id="_name" name="me" value="{{$meeting->title}}">
    <div class="row clearfix">
        <div class="col-sm-12 col-lg-12">
            <div class="row">
                <div class="form-group col-12 col-md-12">
                    <label for="name" class="control-label">Title <span class="text-danger">*</span></label>
                    <input type="text" id="title" name="title" value="{{old('title', $meeting->title)}}" class="form-control">
                    @error('title')
                ...
                <div class="form-group col-12 col-md-4">
                    ...
                    <input type="text" id="start_date" name="start_date" value="{{old('start_date', $meeting->start_date)}}" class="form-control date">
                    @error('start_date')
                    <span class="input-note text-danger">{{ $message }} </span>
                    @enderror
                ...
                <div class="form-group col-12 col-md-4">
                    ...
                    <input type="text" id="end_date" name="end_date" value="{{old('end_date', $meeting->end_date)}}" class="form-control date">
                    @error('end_date')
                    <span class="input-note text-danger">{{ $message }} </span>
                    @enderror
                ...
                <div class="form-group col-12 col-md-4">
                    ...
                    <select name="subsidiary_id" id="subsidiary_id" data-live-search="true" class="form-control selectpicker">
                        ...
                        @foreach($subsidiaries as $subsidiary)
                            <option @if($meeting->subsidiary_id == $subsidiary->id) selected="selected" @endif value="{{ $subsidiary->id }}">{{ $subsidiary->name }}</option>
                        @endforeach
                    </select>
                    @error('subsidiary_id')
                    <span class="input-note text-danger">{{ $message }} </span>
                ...
                <div class="form-group col-12 col-md-4">
                    ...
                    <select name="department_id" id="department_id" data-live-search="true" class="form-control selectpicker">
                        ...
                        @foreach($departments as $department)
                            <option @if($meeting->departemnt_id == $department->id) selected="selected" @endif value="{{ $department->id }}">{{ $department->department_name }}</option>
                        @endforeach
                    </select>
                    @error('department_id')
                    <span class="input-note text-danger">{{ $message }} </span>
                ...
                <div class="form-group col-6 col-md-4">
                    ...
                    <select name="status" id="status" class="form-control selectpicker">
                        <option value="0" @if($meeting->status == 0) selected="selected" @endif>Disable</option>
                        <option value="1" @if($meeting->status == 1) selected="selected" @endif>Enable</option>
                    </select>
                ...
                <div class="form-group col-12">
                    ...
                    <textarea class="form-control ckeditor-classic" rows="9" name="description" id="description">{!! $meeting->description  !!}</textarea>
                    @error('description')
                    <span class="input-note text-danger">{{ $message }} </span>
                    @enderror
...
<script>
    ...
    $('#status').selectpicker('val', '{{$meeting->status}}');
    $('#subsidiary_id').selectpicker('val', '{{$meeting->subsidiary_id}}');
    $('#department_id').selectpicker('val', '{{$meeting->department_id}}');
</script>
