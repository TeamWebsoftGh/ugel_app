<form method="POST" id="complaint" action="{{route('property.complaints.store')}}">
    ...
    <div class="row clearfix">
        @include("shared.form.select-property-type", ["employee_id" => $complaint->complaint_from, "name" => "complaint_from","label" => "Complaint From"])
        @include("shared.form.select-property-type", ["employee_id" => $complaint->complaint_against, "name" => "complaint_against", "label" => "Complaint Against"])
        ...
        <div class="form-group col-6 col-md-4">
            ...
            <input type="text" name="complaint_title" id="complaint_title" required class="form-control"
                   placeholder="{{__('Complaint Title')}}"
                   value="{{old('complaint_title', $complaint->complaint_title)}}">
