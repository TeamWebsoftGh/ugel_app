<form method="POST" action="{{route('parliamentary-candidates.store')}}" id="parliamentary_candidate" >
                ...
                <div class="form-group col-12 col-md-6">
                    ...
                    <select name="constituency_id" id="constituency_id" data-live-search="true" class="form-control selectpicker">
                        @forelse($constituencies as $constituency)
                            <option value="{{$constituency->id}}" @if(old('constituency_id', $candidate->electoral_area?->constituency_id) == $constituency->id) selected @endif>{{$constituency->name}}</option>
                        @empty
                        @endforelse
                    </select>
