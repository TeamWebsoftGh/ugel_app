...
abstract class DocumentTemplate extends Base
{
    ...
    public function __construct(
        ...
        bool $hideItems = false, bool $hideName = false, bool $hideDescription = false, bool $hideQuantity = false, bool $hideDuration = false, bool $hidePrice = false, bool $hideDiscount = false, bool $hideAmount = false, bool $hideNote = false,
        string $textItems = '', string $textQuantity = '', string $textDuration = '', string $textPrice = '', string $textAmount = ''
    ) {
        $this->type = $type;
        $this->item = $item;
        ...
        $this->hideDuration = $this->getHideQuantity($type, $hideDuration);
        ...
        $this->textDuration = $this->getTextDuration($type, $textDuration);
    ...
    protected function getTextDuration($type, $textDuration)
    {
        if (!empty($textDuration)) {
            return $textDuration;
        }

        // if you use setting translation
        if (setting($this->getSettingKey($type, 'duration_name'), 'duration') === 'custom') {
            if (empty($textDuration = setting($this->getSettingKey($type, 'duration_name_input')))) {
                $textDuration = 'invoices.duration';
            }

            return $textDuration;
        }

        if (setting($this->getSettingKey($type, 'duration_name')) !== null &&
            (trans(setting($this->getSettingKey($type, 'duration_name'))) != setting($this->getSettingKey($type, 'duration_name')))
        ) {
            return setting($this->getSettingKey($type, 'duration_name'));
        }

        $translation = $this->getTextFromConfig($type, 'duration');

        if (!empty($translation)) {
            return $translation;
        }

        return 'invoices.duration';
    }
    ...
    protected function getHideDuration($type, $hideDuration)
    {
        ...
        // if you use setting translation
        if ($hideDuration = setting($this->getSettingKey($type, 'hide_duration'), false)) {
            return $hideDuration;
        }

        $hide = $this->getHideFromConfig($type, 'duration');
        ...
        // @todo what return value invoice or always false??
        return setting('invoice.hide_duration', $hideDuration);
    }
