    ...
    public function createUser(array $data)
    {
        ...
        try {
            ...
            if (isset($data['company']))
                $user->companies()->sync($data['company']);
        ...
        $auditMessage = 'You have successfully added a new user: '.$user->name;
...
//            }
            $data['name'] = $user->employee->fullname;
            ...
            $this->updateFinanceUser($user->email, ['name' => $data['name'], 'enabled' => $data['is_active']]);
        ...
        $auditMessage = 'You have successfully updated user: '.$user->name;
    ...
    public function resetPassword(User $user)
    {
        ...
        $auditMessage = "Successfully changed password for " .$user->employee->fullname;
    ...
    public function updateFinanceUser(string $email, array $data)
    {
    ...
    private function deleteFinanceUser($email)
    {
        if(!settings('enable_account', 0))
            return;
        $usr = null;
        try {
            ...
            log_error(format_exception($ex), $usr, "delete-user");
        }
    }
}
