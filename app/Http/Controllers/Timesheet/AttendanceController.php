        ...
        if (request()->ajax())
        {
            ...
            if(!($logged_user->can('view-attendance'))){ //Correction
                ...
                $employee = Employee::with(['officeShift', 'employeeAttendance' => function ($query) use ($selected_date)
                    ...
                    ->select('id', 'company_id', 'department_id', 'first_name', 'last_name', 'office_shift_id')
                    ->where('joining_date', '<=', $selected_date)
                    ->where('is_active',1)
                    ->where('exit_date',NULL)
                    ->get();
...
//        dd($employee_attendance_last);
        ...
        if (!$employee_attendance_last)
        {
            ...
            if ($current_time > $shift_in)
            {
                ...
                //dd($timeDifference > "05:58");
            } // if employee is early or on time
            else
            {
                if(!env('ENABLE_EARLY_CLOCKIN')) {
                    $data['clock_in'] = $current_time->format('H:i');
                }
                else {
        ...
        //if ($employee_attendance_last)
            ...
            if ($employee_attendance_last->clock_in_out == 1) {
                if ($current_time > $shift_in || !env('ENABLE_EARLY_CLOCKIN')) {
                    ...
                    if ($current_time < $shift_out) {
                        ...
                        if($timeDifference > "5:00")
                        {
                            $this->setErrorMessage(__("Clock out is no allowed at this time."));
                            return redirect()->back();
                        }
                    }
...
//                Attendance::create($data);
                $this->setErrorMessage(__('You have already clocked In/Out for today.'));
                return redirect()->back();
            }
        }
        ...
        if (request()->ajax())
        {
            ...
            else
            {
                $employee = Employee::with(['officeShift', 'employeeAttendance' => function ($query) use ($start_date, $end_date)
                {
                    ...
                    ->select('id', 'company_id', 'department_id', 'first_name', 'last_name', 'office_shift_id', 'joining_date')
                    ->where('is_active', '=', 1);
                ...
                foreach ($employee as $key1 => $emp) {
                    ...
                    foreach ($date_range as $key2 => $dt_r) {
                        ...
                        $emp_attendance_date_range[$key1*count($date_range)+$key2]['department_name'] = $emp->department->department_name;
    ...
    public function attendanceHandler($request)
    {
        ...
        if (!$employee_attendance_last)
        {
            ...
            if ($clock_out > $shift_in || !env('ENABLE_EARLY_CLOCKIN')) {
                // if employee is early leaving
