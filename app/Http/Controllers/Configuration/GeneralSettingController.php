...
class GeneralSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        {
            $data = $request->except('_token', '_method', 'logo', 'id', 'favicon', 'whatsapp');
            $validatedData = $request->validate([
                'whatsapp_base_url',
    ...
    public function siteWhatsApp()
    {
        ...
        return view('configuration.settings.whatsapp', compact('rec', 'data'));
    }

    public function updateSiteWhatsApp(Request $request)
    {
        $data = $request->except('_token', '_method', 'id');

        if($request->has('whatsapp_test_number'))
        {
            $validatedData = $request->validate([
                'whatsapp_test_number' => 'required',
            ]);
            try {

                $data['phone_number'] = $data['whatsapp_test_number'];
                $this->sendWhatAppMessages($data['phone_number'], "whatsapp_test", "template");
                request()->session()->flash('message', "WhatsApp message successfully sent.");
            } catch (\Exception $e) {
                request()->session()->flash('error', "Could not send the WhatsApp message. Please check your WhatsApp message settings");
            }
        }
        else{
            $data = $request->except('_token', '_method', 'logo', 'id', 'favicon', 'whatsapp');
